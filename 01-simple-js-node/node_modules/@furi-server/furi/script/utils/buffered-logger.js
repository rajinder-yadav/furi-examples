...
class BufferedLogger {
    ...
    debug(message) {
        if (this.enabled && this.logLevelRank <= types_js_1.LogLevelsRank.DEBUG) {
            this.worker.postMessage({ level: types_js_1.LogLevels.DEBUG, message });
        }
    }
    info(message) {
        if (this.enabled && this.logLevelRank <= types_js_1.LogLevelsRank.INFO) {
            this.worker.postMessage({ level: types_js_1.LogLevels.INFO, message });
        }
    }
    log(message) {
        if (this.enabled && this.logLevelRank <= types_js_1.LogLevelsRank.LOG) {
            this.worker.postMessage({ level: types_js_1.LogLevels.LOG, message });
        }
    }
    warm(message) {
        if (this.enabled && this.logLevelRank <= types_js_1.LogLevelsRank.WARN) {
            this.worker.postMessage({ level: types_js_1.LogLevels.WARN, message });
        }
    }
    error(message) {
        if (this.enabled && this.logLevelRank <= types_js_1.LogLevelsRank.ERROR) {
            this.worker.postMessage({ level: types_js_1.LogLevels.ERROR, message });
        }
    }
    critical(message) {
        if (this.enabled && this.logLevelRank <= types_js_1.LogLevelsRank.CRITICAL) {
            this.worker.postMessage({ level: types_js_1.LogLevels.CRITICAL, message });
        }
    }
    fatal(message) {
        if (this.enabled && this.logLevelRank <= types_js_1.LogLevelsRank.FATAL) {
            this.worker.postMessage({ level: types_js_1.LogLevels.FATAL, message });
        }
    }
}
