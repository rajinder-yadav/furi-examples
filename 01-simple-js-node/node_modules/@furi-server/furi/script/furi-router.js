...
class FuriRouter {
                ...
                for (const route of routerConfig.routes) {
                    ...
                    switch (route.method.toLowerCase()) {
                        ...
                        case 'delete':
                            ...
                            break;
                        case 'options':
                            this.buildRequestMap(types_js_1.HttpMapIndex.OPTIONS, routePath, handlers);
                            break;
                        case 'head':
                            this.buildRequestMap(types_js_1.HttpMapIndex.HEAD, routePath, handlers);
                            break;
                        case 'all':
    ...
    options(uri, ...fn) {
        ...
        return this.buildRequestMap(types_js_1.HttpMapIndex.OPTIONS, uri, fn);
    }
    /**
    ...
    head(uri, ...fn) {
        ...
        return this.buildRequestMap(types_js_1.HttpMapIndex.HEAD, uri, fn);
    }
    /**
    ...
    dispatch(incomingMessage, response) {
        ...
        if (furi_js_1.Furi.fastLogger) {
            furi_js_1.Furi.fastLogger.info(`host: ${request.headers.host}, remote-ip: ${request.socket.remoteAddress}, remote-port: ${request.socket.remotePort}, http: ${request.httpVersion}, method: ${request.method}, url: ${request.url}`);
        }
        switch (request.method) {
            ...
            case 'OPTIONS':
            case 'options':
                this.processHTTPMethod(types_js_1.HttpMapIndex.OPTIONS, request, response);
                break;
            case 'HEAD':
            case 'head':
                this.processHTTPMethod(types_js_1.HttpMapIndex.HEAD, request, response);
                break;
            default:
                ...
                (0, types_js_1.LOG_ERROR)(`FuriRouter::disptch HTTP method ${request.method} is not supported.`);
                response.end();
        } // switch
    }
    /**
        ...
        try {
            if (routeMap.staticRouteMap[URL]) {
                ...
                if (routeMap.namedRoutePartitionMap[bucket]) {
                    ...
                    (0, types_js_1.LOG_WARN)(`FuriRouter::processHTTPMethod Route not found for ${URL}`);
            ...
            (0, types_js_1.LOG_ERROR)('FuriRouter::processHTTPMethod URI Not Found.');
            response.writeHead(404, {
                'Content-Type': 'text/plain',
                'User-Agent': furi_js_1.Furi.getApiVersion(),
            });
